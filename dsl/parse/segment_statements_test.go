...
func TestSegmentStatements(t *testing.T) {
	file := &types.File{Name: "test.fbp", Data: []byte("A -> B\n\n'x' -> IN Sink")}
	cases := []struct {
		name  string
		input []types.Token
		want  []types.Statement
	}{
		{
			name: "splits on eol and ignores empty statements",
			input: []types.Token{
				testToken(file, types.TokIdent, 0, 1, 1, 1, "A"),
				testToken(file, types.TokArrow, 2, 1, 3, 4, "->"),
				testToken(file, types.TokIdent, 5, 1, 6, 6, "B"),
				testToken(file, types.TokEOL, 6, 1, 7, 7, "\n"),
				testToken(file, types.TokEOL, 7, 2, 1, 8, "\n"),
				testToken(file, types.TokQuoted, 8, 3, 1, 11, "'x'"),
				testToken(file, types.TokArrow, 12, 3, 5, 14, "->"),
				testToken(file, types.TokIdent, 15, 3, 8, 17, "IN"),
				testToken(file, types.TokIdent, 18, 3, 11, 22, "Sink"),
				testToken(file, types.TokEOF, 22, 3, 15, 22, file.Name),
			},
			want: []types.Statement{
				{
					Tokens: []types.Token{
					...
					Span: types.Span{File: file.Name, Offset: 0, Line: 1, Column: 1, End: 6},
				},
				{
					Tokens: []types.Token{
						testToken(file, types.TokQuoted, 8, 3, 1, 11, "'x'"),
					...
					Span: types.Span{File: file.Name, Offset: 8, Line: 3, Column: 1, End: 22},
				},
			},
		},
		{
	...
	for i := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			out := make(chan types.Statement, len(tc.want)+1)
			segment.In = in
			segment.Out = out
...
func TestLexerToStatementsPipeline(t *testing.T) {
	...
	file := &types.File{Name: "test.fbp", Data: []byte(src)}
	...
	lexerIn := make(chan *types.File)
	lexerOut := make(chan types.Token)
	...
	stripped := make(chan types.Token, 32)
	statementsOut := make(chan types.Statement, 4)
	...
	want := []types.Statement{
		{
			Tokens: []types.Token{
				testToken(file, types.TokIdent, 20, 2, 1, 24, "Read"),
				testToken(file, types.TokLParen, 24, 2, 5, 25, "("),
				testToken(file, types.TokIdent, 25, 2, 6, 28, "dsl"),
				testToken(file, types.TokSlash, 28, 2, 9, 29, "/"),
				testToken(file, types.TokIdent, 29, 2, 10, 35, "Reader"),
				testToken(file, types.TokRParen, 35, 2, 16, 36, ")"),
				testToken(file, types.TokIdent, 37, 2, 18, 40, "OUT"),
				testToken(file, types.TokArrow, 41, 2, 22, 43, "->"),
				testToken(file, types.TokIdent, 44, 2, 25, 46, "IN"),
				testToken(file, types.TokIdent, 47, 2, 28, 52, "Parse"),
			},
			Span: types.Span{File: file.Name, Offset: 20, Line: 2, Column: 1, End: 52},
		},
		{
			Tokens: []types.Token{
				testToken(file, types.TokQuoted, 54, 4, 1, 61, "'hello'"),
				testToken(file, types.TokArrow, 62, 4, 9, 64, "->"),
				testToken(file, types.TokIdent, 65, 4, 12, 67, "IN"),
				testToken(file, types.TokIdent, 68, 4, 15, 73, "Print"),
			},
			Span: types.Span{File: file.Name, Offset: 54, Line: 4, Column: 1, End: 73},
		},
	}
