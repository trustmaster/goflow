...
func (p *ParseConnection) Process() {
	for stmt := range p.In {
		if proc := tryParseStandaloneProcess(stmt); proc != nil {
			p.Out <- types.Fragment{Kind: types.FragmentProcess, Process: proc}
			continue
		}
...
func tryParseStandaloneProcess(stmt types.Statement) *types.ProcessDef {
	...
	for i := range stmt.Tokens {
		...
		case types.TokArrow, types.TokEqual, types.TokDot, types.TokColon:
			return nil
		}
	}
	...
	if stmt.Tokens[0].Type != types.TokIdent || stmt.Tokens[1].Type != types.TokLParen {
		return nil
	}
	...
	return &types.ProcessDef{Name: procTok.Value, Component: comp}
}

// parseSourceEndpoint parses the source side of a connection: ProcName [(Component)] PortName [[Index]].
func parseSourceEndpoint(cur *tokenCursor) (parseEndpointResult, *types.ParseError) {
...
func parseTargetEndpoint(cur *tokenCursor) (parseEndpointResult, *types.ParseError) {
	var res parseEndpointResult
...
func parseConnectionStatement(stmt types.Statement) ([]types.Fragment, *types.ParseError) {
	cur := newTokenCursor(stmt.Tokens)
	...
	if src.component != "" {
		frags = append(frags, types.Fragment{
			...
			Process: &types.ProcessDef{Name: src.procName, Component: src.component},
		})
	}
	...
	if tgt.component != "" {
		frags = append(frags, types.Fragment{
			Kind:    types.FragmentProcess,
			Process: &types.ProcessDef{Name: tgt.procName, Component: tgt.component},
		})
	}

	frags = append(frags, types.Fragment{
		Kind: types.FragmentConnection,
		Connection: &types.ConnectionDef{
			Src: types.Endpoint{Process: src.procName, Port: src.portName, Index: src.index},
			Tgt: types.Endpoint{Process: tgt.procName, Port: tgt.portName, Index: tgt.index},
		},
	})
...
func parseComponent(cur *tokenCursor) (string, *types.ParseError) {
	if cur.peek().Type != types.TokLParen {
		return "", nil
	}
	...
	for cur.peek().Type == types.TokSlash {
		cur.consume() // consume '/'
	...
	if _, err := cur.expect(types.TokRParen); err != nil {
		return "", err
	}
...
func parseArrayIndex(cur *tokenCursor) (*int, *types.ParseError) {
	if cur.peek().Type != types.TokLBracket {
		return nil, nil
	}

	cur.consume() // consume '['

	tok, err := cur.expect(types.TokInt)
	if err != nil {
		return nil, err
	}
	...
	if _, err := cur.expect(types.TokRBracket); err != nil {
		return nil, err
	}
