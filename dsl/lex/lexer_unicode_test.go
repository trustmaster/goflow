...
func TestUnicodeIdentifiers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			def, err := dsl.ParseDefinition([]byte(tt.input))
			if err != nil {
...
func TestUnicodeQuotedStrings(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			def, err := dsl.ParseDefinition([]byte(tt.input + " -> IN Test"))
			if err != nil {
				t.Fatalf("ParseDefinition error: %v", err)
			}
...
Test(comp)
`
	def, err := dsl.ParseDefinition([]byte(input))
	if err != nil {
		t.Fatalf("ParseDefinition error: %v", err)
	}
...
func TestMultiByteColumnTracking(t *testing.T) {
	...
	input := "你好 42"
	_, err := dsl.ParseDefinition([]byte(input))
	if err == nil {
...
func TestLexerUnicodeTokens(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			src:  "你好 '世界'",
			tokens: []types.Token{
				{Type: types.TokIdent, Pos: 0, Value: "你好", Span: types.Span{File: "test.fbp", Offset: 0, Line: 1, Column: 1, End: 6}},
				{Type: types.TokWhitespace, Pos: 6, Value: " ", Span: types.Span{File: "test.fbp", Offset: 6, Line: 1, Column: 3, End: 7}},
				{Type: types.TokQuoted, Pos: 7, Value: "'世界'", Span: types.Span{File: "test.fbp", Offset: 7, Line: 1, Column: 4, End: 15}},
				{Type: types.TokEOF, Pos: 15, Value: "test.fbp", Span: types.Span{File: "test.fbp", Offset: 15, Line: 1, Column: 8, End: 15}},
			},
		},
		{
			...
			src:  "A(组件/测试)",
			tokens: []types.Token{
				{Type: types.TokIdent, Pos: 0, Value: "A", Span: types.Span{File: "test.fbp", Offset: 0, Line: 1, Column: 1, End: 1}},
				{Type: types.TokLParen, Pos: 1, Value: "(", Span: types.Span{File: "test.fbp", Offset: 1, Line: 1, Column: 2, End: 2}},
				{Type: types.TokIdent, Pos: 2, Value: "组件", Span: types.Span{File: "test.fbp", Offset: 2, Line: 1, Column: 3, End: 8}},
				{Type: types.TokSlash, Pos: 8, Value: "/", Span: types.Span{File: "test.fbp", Offset: 8, Line: 1, Column: 5, End: 9}},
				{Type: types.TokIdent, Pos: 9, Value: "测试", Span: types.Span{File: "test.fbp", Offset: 9, Line: 1, Column: 6, End: 15}},
				{Type: types.TokRParen, Pos: 15, Value: ")", Span: types.Span{File: "test.fbp", Offset: 15, Line: 1, Column: 8, End: 16}},
				{Type: types.TokEOF, Pos: 16, Value: "test.fbp", Span: types.Span{File: "test.fbp", Offset: 16, Line: 1, Column: 9, End: 16}},
			},
		},
	}
	...
	for i := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for idx := range tc.tokens {
				if !lex.LexerTokEql(tokens[idx], tc.tokens[idx]) {
